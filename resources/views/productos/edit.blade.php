...
@section('content')
    <div class="mb-6 flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-label">Editar producto</h1>
    </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="unidad" name="unidad"
                        ...
                        @foreach (['unidad', 'kg', 'g', 'litro', 'ml', 'caja', 'paquete', 'par'] as $u)
                            <option value="{{ $u }}" {{ old('unidad', $producto->unidad) === $u ? 'selected' : '' }}>
                                {{ ucfirst($u) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-2">
                    ...
                    <textarea id="descripcion" name="descripcion" rows="3"
                        class="w-full bg-system-background border rounded-lg px-3 py-2 text-sm text-label focus:outline-none focus:ring-2 focus:ring-system-blue
                            {{ $errors->has('descripcion') ? 'border-system-red' : 'border-separator' }}">{{ old('descripcion', $producto->descripcion) }}</textarea>
